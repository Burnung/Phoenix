...
//using namespace std;
P3DCamera::P3DCamera(float *tmp, int width, int height) :mouseClick(false){
...
P3DCamera::P3DCamera():Oposition(glm::vec3(0,0,0)),Otarget(glm::vec3(0,0,-1.0f)),OupVec(glm::vec3(0,1,0)),
                 m_windowHeight(512),m_windowWidth(512),mouseClick(false){
	Init();
}

P3DCamera::P3DCamera(glm::vec3 &position, glm::vec3 &target_, glm::vec3 &upve_, int w, int h):mouseClick(false){
...
void P3DCamera::OnKeyBoard(int key){
...
void P3DCamera::MoveForward(){
	Nposition += myStep*NtargetVec;
}

void P3DCamera::MoveBack(){
	Nposition -= myStep*NtargetVec;
}

void P3DCamera::MoveLeft(){
	glm::vec3 left = cross(NtargetVec, NupVec);
...
void P3DCamera::OnMouse(float x, float y){
...
void P3DCamera::MousePush(float x,float y){
	//mouspush = !mouspush;
	mouseClick = true;
	MouseX = x;
	MouseY = y;
...
glm::vec3 Rotate(glm::vec3 rource, float angle, glm::vec3 Axe){
	...
	P3DQuaternion RotationQ(Rw, Rx, Ry, Rz);

	P3DQuaternion ConjugateQ = RotationQ.Conjugate();

	ConjugateQ.Normalize();
	P3DQuaternion W = (RotationQ * rource) * ConjugateQ;

	glm::vec3 result(W.x, W.y, W.z);
	return result;
}

void P3DCamera::SetWidth(int w){
	m_windowWidth = w;
}
void P3DCamera::SetHeithg(int h){
	m_windowHeight = h;
}

void P3DCamera::SetPostion(glm::vec3& P){
	Oposition = P;
}

void P3DCamera::SetLookVec(glm::vec3& LV){
	Otarget = LV + Oposition;
}

void P3DCamera::SetUpVec(glm::vec3& up){
	OupVec = up;
}
